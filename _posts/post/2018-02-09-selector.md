...
继承`LeaderSelectorListenerAdapter`，实现`LeaderSelectorListenerAdapter#takeLeadership`方法，该方法由curator框架负责在当前客户端节点成为`leader`之后回调。<br/>
需要注意，`LeaderSelectorListenerAdapter#takeLeadership`方法推出则代表当前节点归还`leader`权限，当你希望该节点长期持有`leader`权限时，请不要退出该方法。<br/>
