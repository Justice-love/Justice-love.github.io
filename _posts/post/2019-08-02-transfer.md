...
需要对Runnable字节码层面进行扩展，一般有两种编制手段，编译时编制和运行时编制，我们平常经常使用的比较多的Spring AOP就是运行时编制，但这存在一种限制，Bean的实例化需要交由容器来实现，主动创建出来的无法进行字节码扩展。

因为Runnable绝大多数的使用方式都是直接创建匿名内部类，所以编制方式不适合运行时编制，而是采用编译时编制，使用AspectJ对其进行代理，AJC进行编译。
